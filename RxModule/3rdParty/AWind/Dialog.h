...
public:
	...
	*/
    Dialog(const char * caption,int left,int top,int width,int height):Window(caption,left,top,width,height)
	{
		_btnOK=NULL;
		_btnCancel=NULL;
		_dialogClosedEventReceiver=NULL;
		SetDecorators(Environment::Get()->FindDecorators(("Dialog")));
		TextBoxFString *captionText=new TextBoxFString(2,2,width-4,20,caption);
		captionText->SetDecorators(Environment::Get()->FindDecorators(("DialogCaption")));
	...
	\param height window height
	*/
    Dialog(int left,int top,int width,int height):Window(("Dialog"),left,top,width,height)
	{
		_btnOK=NULL;
		_btnCancel=NULL;
		_dialogClosedEventReceiver=NULL;
